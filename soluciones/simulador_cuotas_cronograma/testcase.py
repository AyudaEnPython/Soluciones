...
class Test(TestCase):
    ...
    def test_Cronograma_data(self):
        ...
        for i in range(0, len(self.cronograma.data)):
            self.assertEqual(
                self.cronograma.data[i]["periodo"],
                data[i]["periodo"],
            )
            self.assertEqual(
                self.cronograma.data[i]["fecha_pago"],
                data[i]["fecha_pago"],
            )
            self.assertEqual(
                self.cronograma.data[i]["fecha_vencimiento"],
                data[i]["fecha_vencimiento"],
            )
            self.assertEqual(
                self.cronograma.data[i]["dias"],
                data[i]["dias"],
            )
            self.assertEqual(
                self.cronograma.data[i]["capital"],
                data[i]["capital"],
            )
            self.assertEqual(
                self.cronograma.data[i]["amortizacion"],
                data[i]["amortizacion"],
            )
            self.assertEqual(
                self.cronograma.data[i]["interes"],
                data[i]["interes"]),
            self.assertEqual(
                self.cronograma.data[i]["cuota"],
                data[i]["cuota"],
            )
