...
# js
# console.log(Array.from({length: 100}, (v, k) => k+1).filter((n) => ![...Array(n).keys()].slice(2).map(i => !(n%i)).includes(true) && ![0,1].includes(n)).reduce((x, y) => x + y));    # noqa: E501, E702

# python
from functools import reduce; print(reduce(lambda x, y: x + y, list(filter(lambda x: all(x % i != 0 for i in range(2, x)), list(range(1, 101))[1:]))))  # noqa: E501, E702
